...
const ProgressPhotography: React.FC = () => {
  ...
  return <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <div className="flex items-center gap-2">
              <Camera className="h-4 w-4" style={{
              color: '#737373'
            }} />
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {moods.map(mood => <Badge key={mood} variant={selectedMood === mood ? "default" : "outline"} className="cursor-pointer" onClick={() => setSelectedMood(mood)}>
                  {mood}
                </Badge>)}
            </div>
          </div>
      ...
      {photos.length > 0 && <Card className="p-6 bg-white/90">
          <h4 className="text-lg font-semibold mb-4" style={{
        color: '#737373'
      }}>Your Mood Board</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {photos.map(photo => <div key={photo.id} className="border rounded-lg p-3 bg-mental-peach/20">
              ...
              color: '#737373'
                  ...
                  {photo.tags.length > 0 && <div className="flex flex-wrap gap-1">
                      {photo.tags.map((tag, index) => <Badge key={index} variant="outline" className="text-xs">
                          {tag}
                        </Badge>)}
                    </div>}

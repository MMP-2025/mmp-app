  ...
  return (
    <div className="space-y-6">
      <Card className="p-6 bg-mental-gray">
        <h3 className="text-xl font-semibold mb-4 text-foreground">Progress Photography</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <Label className="mb-2 block text-muted-foreground">Upload Photo</Label>
          ...
          <div>
            <Label className="mb-2 block text-muted-foreground">Current Mood</Label>
            <div className="flex flex-wrap gap-2">
              {moods.map(mood => (
                <Badge key={mood} variant={selectedMood === mood ? "default" : "outline"} className="cursor-pointer" onClick={() => setSelectedMood(mood)}>
                  {mood}
                </Badge>
              ))}
            </div>
          ...
          <div>
            <Label className="mb-2 block text-muted-foreground">Caption</Label>
            <Textarea placeholder="Describe what this moment means to you..." value={currentCaption} onChange={e => setCurrentCaption(e.target.value)} className="h-20" />
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <Input placeholder="nature, progress, breakthrough..." value={currentTags} onChange={e => setCurrentTags(e.target.value)} />
            </div>
          </div>
        </div>
      ...
      {photos.length > 0 && (
        <Card className="p-6 bg-white/90">
          <h4 className="text-lg font-semibold mb-4 text-foreground">Your Mood Board</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {photos.map(photo => (
              <div key={photo.id} className="border rounded-lg p-3 bg-mental-peach/20">
                <img src={photo.image} alt={photo.caption} className="w-full h-48 object-cover rounded mb-3" />

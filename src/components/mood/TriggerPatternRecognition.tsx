...
const TriggerPatternRecognition: React.FC<TriggerPatternRecognitionProps> = ({
  ...
  const analyzeTriggerPatterns = () => {
    ...
    setTimeout(() => {
        ...
        if (entry.intensity <= 4) {
          entry.factors.forEach(factor => {
            if (!triggerCounts[factor.toLowerCase()]) {
              triggerCounts[factor.toLowerCase()] = {
                count: 0,
                moodImpacts: [],
                factors: [],
                recentCount: 0
      ...
      const patterns: TriggerPattern[] = Object.entries(triggerCounts).filter(([_, data]) => data.count >= 2).map(([trigger, data]) => {
        const averageMoodImpact = data.moodImpacts.length > 0 ? data.moodImpacts.reduce((sum, impact) => sum + impact, 0) / data.moodImpacts.length : 5;
        const uniqueFactors = [...new Set(data.factors)];
        let severity: 'low' | 'medium' | 'high' = 'low';
        if (data.count >= 5 && averageMoodImpact <= 3) severity = 'high';else if (data.count >= 3 && averageMoodImpact <= 4) severity = 'medium';
        return {
  ...
  return <Card className="p-6 bg-mental-blue">
      <div className="flex items-center gap-2 mb-4">
        ...
        color: '#737373'
      }}>AI Trigger Pattern Recognition</h3>
      </div>
      
      <TriggerAnalysisButton onAnalyze={analyzeTriggerPatterns} isAnalyzing={isAnalyzing} canAnalyze={canAnalyze} />

      {triggers.length > 0 && <div className="space-y-4">
          <h4 className="font-semibold" style={{
        color: '#737373'
      }}>Identified Patterns</h4>
          {triggers.map((trigger, index) => <TriggerPatternCard key={index} trigger={trigger} />)}
        </div>}

      {triggers.length === 0 && !isAnalyzing && <div className="text-center py-8">
          <p style={{
        color: '#737373'
      }}>
            {!canAnalyze ? 'Keep tracking your mood and writing journal entries to unlock AI trigger analysis.' : 'No significant trigger patterns detected yet. Continue tracking to improve accuracy.'}
          </p>
        </div>}
    </Card>;
};

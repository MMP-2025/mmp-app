...
const MoodAnalytics = ({
  ...
  const analytics = useMemo(() => {
    ...
    const topFactors = Object.entries(factorCounts).sort(([, a], [, b]) => b - a).slice(0, 10).map(([factor, count]) => ({
      factor,
      count
    }));
    ...
    const weeklyTrend = last7Days.map(date => {
      ...
      const avgIntensity = dayEntries.length > 0 ? dayEntries.reduce((sum, entry) => sum + entry.intensity, 0) / dayEntries.length : 0;
      return {
  ...
  if (!analytics) {
    return <Card className="p-6 bg-mental-blue">
        ...
        <p style={{
        color: '#737373'
      }}>Start tracking your mood to see analytics and insights!</p>
      </Card>;
  }
  return <div className="space-y-6">
      <MoodAnalyticsStats totalEntries={analytics.totalEntries} currentStreak={analytics.currentStreak} avgIntensity={analytics.avgIntensity} topFactor={analytics.topFactors[0]?.factor || 'N/A'} />

...
const HabitTracker: React.FC<HabitTrackerProps> = ({ onHabitUpdate }) => {
  const [sleepHours, setSleepHours] = useState<number[]>([7]);
  const [exerciseMinutes, setExerciseMinutes] = useState<number[]>([30]);
  const [exerciseCompleted, setExerciseCompleted] = useState(false);
  const [medications, setMedications] = useState<Array<{name: string; taken: boolean; time?: string}>>([]);
  const [customHabits, setCustomHabits] = useState<Array<{name: string; completed: boolean}>>([]);
  ...
  const addMedication = () => {
    ...
    setMedications(prev => [...prev, { name: newMedication, taken: false }]);
    setNewMedication('');
  };

  const addCustomHabit = () => {
    if (!newHabit.trim()) return;
    setCustomHabits(prev => [...prev, { name: newHabit, completed: false }]);
    setNewHabit('');
  };

  const toggleMedication = (index: number) => {
    setMedications(prev => prev.map((med, i) => 
      i === index ? { ...med, taken: !med.taken, time: !med.taken ? new Date().toLocaleTimeString() : undefined } : med
    ));
  };

  const toggleCustomHabit = (index: number) => {
    setCustomHabits(prev => prev.map((habit, i) => 
  ...
  return (
    <Card className="p-6 bg-white/90">
      <h3 className="text-xl font-semibold mb-4" style={{color: '#737373'}}>Daily Habits</h3>
      
      <div className="space-y-6">
        {/* Sleep Tracking */}
        <div>
          <div className="flex items-center gap-2 mb-3">
            <Moon className="h-5 w-5" style={{color: '#737373'}} />
            <Label className="text-lg font-medium" style={{color: '#737373'}}>
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            <Dumbbell className="h-5 w-5" style={{color: '#737373'}} />
            <Label className="text-lg font-medium" style={{color: '#737373'}}>
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            <Pill className="h-5 w-5" style={{color: '#737373'}} />
            <Label className="text-lg font-medium" style={{color: '#737373'}}>Medications</Label>
          </div>
          ...
          <div className="space-y-2">
            {medications.map((med, index) => (
              <div key={index} className="flex items-center justify-between p-2 bg-mental-peach/20 rounded">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    checked={med.taken}
        ...
        {/* Custom Habits */}
        <div>
          <Label className="text-lg font-medium mb-3 block" style={{color: '#737373'}}>Custom Habits</Label>
          <div className="flex gap-2 mb-3">
            <Input

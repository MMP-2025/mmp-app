...
const CopingSkillsLibrary: React.FC = () => {
  ...
  const defaultSkills: CopingSkill[] = [{
    ...
    steps: ['Sit comfortably with your back straight', 'Inhale through your nose for 4 counts', 'Hold your breath for 7 counts', 'Exhale through your mouth for 8 counts', 'Repeat 3-4 cycles'],
    ...
    steps: ['Notice when negative thoughts start', 'Say "STOP" out loud or mentally', 'Take 3 deep breaths', 'Replace with a positive or neutral thought', 'Engage in a different activity'],
    ...
    steps: ['Lie down or sit comfortably', 'Start with your toes - tense for 5 seconds, then relax', 'Move up to your calves, thighs, abdomen', 'Continue with arms, shoulders, face', 'Notice the contrast between tension and relaxation'],
    ...
    difficulty: 'medium',
    duration: '10 minutes',
    steps: ['Identify what you\'re feeling', 'Say "It\'s okay to feel this way"', 'Remember emotions are temporary', 'Ask what this emotion is telling you', 'Decide on a healthy response'],
    ...
    difficulty: 'easy',
    duration: 'Variable',
    steps: ['Identify trusted friends or family', 'Choose how to reach out (call, text, in-person)', 'Be specific about what you need', 'Listen to their perspective', 'Express gratitude for their support'],
  ...
  const getPersonalizedRecommendations = () => {
    ...
    const effectiveSkills = recentUsage.filter(usage => usage.effectiveness >= 8).map(usage => usage.skillId);
    const categories = effectiveSkills.map(skillId => skills.find(skill => skill.id === skillId)?.category).filter(Boolean);
    const mostEffectiveCategory = categories.reduce((acc, cat) => {
      acc[cat!] = (acc[cat!] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    const topCategory = Object.entries(mostEffectiveCategory).sort(([, a], [, b]) => b - a)[0]?.[0];
    return skills.filter(skill => skill.category === topCategory && !effectiveSkills.includes(skill.id)).slice(0, 3);
  };
  const filteredSkills = skills.filter(skill => {
    const matchesSearch = skill.title.toLowerCase().includes(searchTerm.toLowerCase()) || skill.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return <div className="space-y-6">
      <Card className="p-6 bg-mental-gray">
        ...
        <div className="flex gap-4 mb-4">
          <Input placeholder="Search skills or tags..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="flex-1" />
        </div>
        ...
        <Tabs value={selectedCategory} onValueChange={setSelectedCategory}>
          ...
          <TabsContent value={selectedCategory} className="mt-4">
            {recommendations.length > 0 && <div className="mb-6">
              ...
              color: '#737373'
                ...
                <div className="grid gap-3">
                  {recommendations.map(skill => <div key={skill.id} className="border rounded-lg p-3 bg-mental-green/10">
              ...
              return <Card key={skill.id} className="p-4">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        {skill.effectiveness && <Badge variant="outline" className="text-green-600">
                  ...
                  color: '#737373'
                    ...
                    color: '#737373'
                      ...
                      <ol className="text-sm space-y-1" style={{
                    color: '#737373'
                  }}>
                        {skill.steps.map((step, index) => <li key={index} className="flex gap-2">
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex flex-wrap gap-1">
                        {skill.tags.map(tag => <Badge key={tag} variant="secondary" className="text-xs">
                            {tag}
                          </Badge>)}
                      </div>
                      
                      <div className="flex items-center gap-2">
                        {skill.timesUsed > 0 && <span className="text-xs" style={{
                      color: '#737373'
                    }}>

...
const ExposureTherapyTracker: React.FC = () => {
  ...
  const createGoal = () => {
    ...
    const steps: ExposureStep[] = newGoal.steps.filter(step => step.trim()).map((step, index) => ({
  ...
  return <div className="space-y-6">
      <Card className="p-6 bg-mental-gray">
        ...
        {showCreateGoal && <div className="border rounded-lg p-4 mb-4 bg-mental-peach/20">
            <h4 className="font-semibold mb-3" style={{
          color: '#737373'
        }}>Create Exposure Goal</h4>
            <div className="space-y-4">
              <div>
                <Label className="mb-2 block" style={{
              color: '#737373'
            }}>Goal Title</Label>
                <Input placeholder="e.g., Speaking in public" value={newGoal.title} onChange={e => setNewGoal(prev => ({
              ...prev,
              ...
              color: '#737373'
                ...
                <Textarea placeholder="Describe what you want to overcome..." value={newGoal.description} onChange={e => setNewGoal(prev => ({
              ...prev,
              ...
              color: '#737373'
                ...
                <Slider value={newGoal.fearLevel} onValueChange={value => setNewGoal(prev => ({
              ...prev,
              fearLevel: value
            }))} min={1} max={10} step={1} />
              </div>
              ...
              color: '#737373'
                ...
                <Input type="date" value={newGoal.targetDate} onChange={e => setNewGoal(prev => ({
              ...prev,
              ...
              <div>
                <Label className="mb-2 block" style={{
              color: '#737373'
            }}>Exposure Steps (easiest to hardest)</Label>
                {newGoal.steps.map((step, index) => <Input key={index} className="mb-2" placeholder={`Step ${index + 1}`} value={step} onChange={e => updateStep(index, e.target.value)} />)}
      ...
      {goals.map(goal => <Card key={goal.id} className="p-6 bg-white/90">
          ...
          <div className="space-y-3">
            {goal.steps.map((step, index) => <div key={step.id} className="flex items-center justify-between p-3 border rounded">
                <div className="flex items-center gap-3">
                  {step.completed ? <CheckCircle className="h-4 w-4 text-green-600" /> : <div className="h-4 w-4 border rounded-full" />}
                  <div>
                    <span className="text-sm" style={{
                color: '#737373'
                    ...
                    <div className="flex gap-2 mt-1">
                      ...
                      {step.completed && <Badge variant="outline" className="text-xs text-green-600">
                          Completed {step.completedDate}
                        </Badge>}
                    </div>
                  </div>
                </div>
                {!step.completed && <Button size="sm" onClick={() => startSession(goal.id, step.id)}>
                    Start Practice
                  </Button>}
              </div>)}
          </div>
      ...
      {showSession && <Card className="p-6 bg-white/90">
        ...
        color: '#737373'
          ...
          <div className="space-y-4">
            ...
            color: '#737373'
              ...
              <Slider value={sessionData.preFear} onValueChange={value => setSessionData(prev => ({
            ...prev,
            preFear: value
          }))} min={0} max={10} step={1} />
            </div>
            ...
            color: '#737373'
              ...
              <Input type="number" value={sessionData.duration} onChange={e => setSessionData(prev => ({
            ...prev,
            ...
            color: '#737373'
              ...
              <Slider value={sessionData.postFear} onValueChange={value => setSessionData(prev => ({
            ...prev,
            postFear: value
          }))} min={0} max={10} step={1} />
            </div>
            ...
            color: '#737373'
              ...
              <Textarea placeholder="How did it go? What did you learn?" value={sessionData.notes} onChange={e => setSessionData(prev => ({
            ...prev,

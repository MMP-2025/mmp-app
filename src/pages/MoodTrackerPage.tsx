...
const MoodTrackerPage = () => {
  ...
  return <div className="bg-mental-blue min-h-screen">
      <SidebarLayout>
        <div className="space-y-6 max-w-4xl mx-auto">
          ...
          <Card className="p-6 bg-white/90">
            ...
            <div className="grid grid-cols-5 gap-2 text-center mb-6">
              ...
              name: 'Angry',
              icon: Angry
            }].map(mood => <button key={mood.name} onClick={() => handleMoodSelection(mood.name)} className={`p-4 rounded-md transition-all flex flex-col items-center gap-2 ${selectedMood === mood.name ? 'bg-mental-peach border-2 border-gray-600' : 'hover:bg-mental-peach/40'}`}>
                  <mood.icon className="h-8 w-8 text-gray-700" />
                  <span className="text-gray-700">{mood.name}</span>
                </button>)}
            </div>

            {selectedMood && <div className="space-y-4">
                <div>
                  <h3 className="text-lg font-medium mb-2 text-gray-800">
                    Tell us more about feeling {selectedMood.toLowerCase()}...
                  </h3>
                  <Textarea placeholder="What's on your mind? How are you feeling right now?" value={moodNote} onChange={e => setMoodNote(e.target.value)} className="min-h-[120px] border-gray-300 focus:border-gray-600 text-gray-800" />
                </div>
                <Button onClick={handleSaveMood} className="bg-mental-green text-white hover:bg-mental-green/80">
                  Save Mood Entry
                </Button>
              </div>}
          </Card>
